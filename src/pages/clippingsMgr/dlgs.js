...
function clippingsMgrDlgs()
{
  ...
  {
    $("#custom-plchldr-name").prop("placeholder", messenger.i18n.getMessage("placeholderNameHint"));
  ...
  {
    $("#numeric-plchldr-name").prop("placeholder", messenger.i18n.getMessage("placeholderNameHint"));
  ...
  importFromFile.onInit = function ()
  {
    if (this.mode == this.IMP_REPLACE) {
      $("#import-clippings-label").text(messenger.i18n.getMessage("labelSelBkupFile"));
      ...
      $("#import-dlg-action-btn").text(messenger.i18n.getMessage("btnRestoreBkup"));
    }
    else {
      $("#import-clippings-label").text(messenger.i18n.getMessage("labelSelImportFile"));
      $("#import-clippings-replc-shct-keys-checkbox").show();
      $("#restore-backup-warning").hide();
      $("#import-dlg-action-btn").text(messenger.i18n.getMessage("btnImport"));
    }
  ...
  importFromFile.onAccept = function (aEvent)
  {
    ...
    function importFile(aAppendItems)
    {
      ...
      fileReader.addEventListener("load", aEvent => {
        ...
        log("Clippings: clippingsMgr/dlgs.js: importFromFile.onAccept()::importFile(): Importing Clippings data asynchronously.");
        ...
        importConfirmMsgBox.setMessage(messenger.i18n.getMessage("clipMgrImportConfirm", importFile.name));
    ...
    function saveToFile(aBlobData, aFilename)
    {
      ...
      messenger.downloads.download(downldOpts).then(aDownldItemID => {
        ...
        if (aDownldItems && aDownldItems.length > 0) {
          ...
          exportConfirmMsgBox.setMessage(messenger.i18n.getMessage("clipMgrExportConfirm", exportFilePath));
  ...
  backupConfirmMsgBox.onAfterAccept = function ()
  {
    if (gIsBackupMode) {
      closeWnd();
    }
  ...
  moveTo.onAccept = function (aEvent)
  {
    ...
    log(`clippingsMgr/dlgs.js: Move To dialog: ID of selected item: ${id}; it is ${selectedNode.isFolder()} that the selected item in the clippings tree is a folder; current parent of selected item: ${parentFolderID}; move or copy to folder ID: ${destFolderID}`);
    ...
    if (parentFolderID == destFolderID && !makeCopy) {
      $("#move-error").text(messenger.i18n.getMessage("errMoveToSameParent"));
      return;
    }

    // Handle case where selected folder and destination folder are the same.
    if (selectedNode.isFolder() && id == destFolderID) {
      $("#move-error").text(messenger.i18n.getMessage("errMoveToSubfldr"));
      return;
    }
