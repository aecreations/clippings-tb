...
let gCmd = {
  ...
  newFolder: function (aDestUndoStack)
  {
    ...
    let newFolder = {
      name: messenger.i18n.getMessage("newFolder"),
      parentFolderID,
      displayOrder,
    };
  ...
  async gotoURL(aURL)
  {
    ...
    try {
      let openerWnd = await messenger.windows.get(gOpenerWndID);
  ...
  async toggleMaximize()
  {
    ...
    let updWnd = messenger.windows.update(messenger.windows.WINDOW_ID_CURRENT, updWndInfo);
    ...
    aeImportExport.exportToJSON(INCLUDE_SRC_URLS, false, aeConst.ROOT_FOLDER_ID, excludeSyncFldrID, true).then(aJSONData => {
      ...
      messenger.downloads.download({
        ...
        if (aErr.fileName == "undefined") {
          ...
          console.error(aErr);
          setStatusBarMsg(messenger.i18n.getMessage("statusSavingBkupFailed"));
...
$(async () => {
  ...
  if (gClippings) {
    ...
    log("Clippings/mx::clippingsMgr: Successfully opened Clippings DB");
  }
  else {
  ...
  gDialogs.importFromFile.onAccept = aEvent => {
	...
	gCmd.recentAction = gCmd.ACTION_RESTORE_BACKUP;
        ...
        gClippingsDB.folders.each((aItem, aCursor) => {
          ...
          log("Clippings/mx::clippingsMgr.js: Finished deleting clippings and folders. Starting import of backup file.");
          importFile(false);
        });
      }).catch(aErr => {
...
function handlePushSyncItemsError(aError)
{
  if (aError == aeConst.SYNC_ERROR_CONXN_FAILED && !gErrorPushSyncItems) {
    ...
    errorMsgBox.onInit = () => {
      $("#sync-error-msgbox > .dlg-content > .msgbox-error-msg").text(messenger.i18n.getMessage("syncPushFailed"));
    };
    errorMsgBox.showModal();
