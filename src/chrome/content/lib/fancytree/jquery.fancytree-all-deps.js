...
function Fancytree(widget) {
	...
	// Create root markup
	$ul = $("<ul>", {  // Changing this to "<html:ul>" will cause an error!
...
//	            node.debug("render... really");
				firstTime = true;
				node.li = document.createElementNS("http://www.w3.org/1999/xhtml", "li");
				node.li.ftnode = node;

				if( node.key && opts.generateIds ){
					node.li.id = opts.idPrefix + node.key;
				}
				node.span = document.createElementNS("http://www.w3.org/1999/xhtml", "span");
...
//				this.nodeRenderTitle(ctx);
		...
		if( children ){
			if( isRootNode || node.expanded || deep === true ) {
				...
				if( !node.ul ){
					node.ul = document.createElementNS("http://www.w3.org/1999/xhtml", "ul");
...
$.ui.fancytree.registerExtension({
	...
	treeCreate: function(ctx){
		...
		var containerId, cssText, iconSpacingUnit, labelSpacingUnit, iconWidthUnit, levelOfsUnit,
			...
			// css sniffing
			$dummyLI = $("<html:li id='fancytreeTemp'><html:span class='fancytree-node'><html:span class='fancytree-icon' /><html:span class='fancytree-title' /></html:span><html:ul />")
